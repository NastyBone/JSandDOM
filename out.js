(() => {
  ...
  function resetPreview() {
    ...
    [...document.getElementsByClassName("container-box")].forEach((e) => {
      e.style["box-shadow"] = "4px 12px 25px -8px #000, 0px 3px 8px -2px #aaa";
    });
    myHandler.setImage("");
  ...
  function substituteElement(parent, target, text = "", event = "", func = "", display = "") {
    ...
    let newElement2 = oldElement.cloneNode(true);
    if (newElement2.tagName == "A")
      newElement2.removeAttribute("onmousedown");
    if (text)
      newElement2.innerText = text;
    if (display)
      newElement2.style.display = display;
    if (event && func)
      newElement2.addEventListener(event, func);
    parent.replaceChild(newElement2, oldElement);
  }
  ...
  async function newElement() {
    let containerData2 = [...document.getElementsByClassName("preview-field-container")];
    ...
    let logicAux = containerData2.filter((e) => {
  ...
  function saveEditedElement() {
    let selectElement2 = myHandler.getSelected();
    let newData = [...document.getElementsByClassName("preview-field-container")];
    selectElement2.childNodes[1].innerHTML = "";
    newData.forEach((element) => {
      ...
      elementContainer.appendChild(elementTag);
      elementContainer.appendChild(elementValue);
      selectElement2.childNodes[1].appendChild(elementContainer);
    });
  ...
  function showSort() {
    ...
    if (previewArea.innerHTML !== "") {
      ...
      [...document.getElementsByClassName("container-box")].forEach((boxData) => [...boxData.childNodes[1].childNodes].forEach((containerData2) => {
        if (containerData2.childNodes[0].className == "element-property-name") {
          let tag = containerData2.childNodes[0].innerText;
          tag = removeDot(tag);
          sortOptions.push(tag);
        }
      }));
      sortOptions = sortOptions.filter((ele, i, self) => self.indexOf(ele) == i);
      sortOptions.map((containerData2) => {
        let button = document.createElement("button");
        button.className = "navbar-button texture-background hoverable preview-sort-button";
        button.innerText = containerData2;
  ...
  function dataToJSON(bigDataBox, dataBox = "") {
    if (dataBox) {
      ...
      bigArr = dataBox.map((container, i) => {
        ...
        auxObj.data = {};
        [...container.childNodes].map((containerData2) => {
          if (containerData2.childNodes[0].className == "element-property-name") {
            headers.push(removeDot(containerData2.childNodes[0].innerText));
          }
          auxObj.data[removeDot(containerData2.childNodes[0].innerText)] = containerData2.childNodes[1].innerText;
        });
      ...
      bigDataBox.forEach((element) => {
        auxObj[element.childNodes[0].value || element.childNodes[0].innerText] = element.childNodes[1].value;
      });
      return auxObj;
    }
  }
  ...
  function addImage(e) {
    if (e.target.files[0])
      myHandler.setImage(e.target.files[0]);
    ...
    function getData(xls) {
      return new Promise((resolve) => {
        ...
        reader.onload = function() {
          ...
          var wb = readSync(fileData, {type: "binary"});
          wb.SheetNames.forEach(function(sheetName) {
            var rowObj = utils.sheet_to_row_object_array(wb.Sheets[sheetName]);
